...
export const newBillCommand = async (ctx) => {
    try {
...
export const handleBillCreation = async (ctx) => {
    try {
        const userId = ctx.from?.id;
        if (!userId)
            return;
...
export function calculateBillSplit(billData) {
...
export async function saveBillAndShowResult(ctx, billData, result) {
    try {
        // Save to database
        const bill = new Bill({
        ...
        const paidPeople = result.results.filter((r) => r.paid > 0);
        if (paidPeople.length > 0) {
            message += `${burmeseMessages.paidPeopleList}\n`;
            paidPeople.forEach((r, index) => {
...
//# sourceMappingURL=newbill.js.map